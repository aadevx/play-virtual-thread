...
public class TestRunner extends Controller {
    ...
    public static void list(Boolean runUnitTests, Boolean runFunctionalTests, Boolean runSeleniumTests) {
        ...
        p.println(Router.reverse(new File(Play.modules.get("_testrunner"), "/public/test-runner/selenium/TestRunner.html")));
        
        List<Class> unitTests = null;
        List<Class> functionalTests =  null;
        List<String> seleniumTests = null;
        // Check configuration of tests
    ...
    public static void run(String test) throws Exception {
        ...
        if (test.endsWith(".test.html.suite")) {
            ...
            renderArgs().put("test", test);
            renderTemplate("TestRunner/selenium-suite.html");
        }
        if (test.endsWith(".test.html")) {
            ...
            if (!testFile.exists()) {
                for(File root : Play.roots) {
                    File moduleTestFile = Play.getFile(FileUtils.relativePath(root)+"/test/" + test);
            ...
            if (testFile.exists()) {
                Template testTemplate = TemplateLoader.load(testFile);
                Map<String, Object> options = new HashMap<String, Object>();
                response().contentType = "text/html";
