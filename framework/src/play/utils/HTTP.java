...
//           try {
                  LocalDateTime browserDate = LocalDateTime.from(Utils.httpFormatter.parse(ifModifiedSince));
//                Date browserDate = Utils.getHttpDateFormatter().parse(ifModifiedSince)
                // Only compare up to the second because the datetime format we send to the client
                // does not have milliseconds
                long ifModifiedSinceDateSeconds = browserDate.getSecond();
                long fileLastModifiedSeconds = last / 1000;
