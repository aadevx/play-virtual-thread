...
public class Utils {
    ...
    public static String open(String file, Integer line) {
        if (Play.configuration.containsKey("play.editor")) {
            File vfile = FileUtils.fromRelativePath(file);
            if (vfile != null) {
                return String.format(Play.configuration.getProperty("play.editor"), vfile.getAbsolutePath(), line);
            }
        }
        return null;
    }
    ...
    public static final DateTimeFormatter httpFormatter = DateTimeFormatter.ofPattern("EEE, dd MMM yyyy HH:mm:ss zzz", Locale.US).withZone(ZoneId.of("GMT"));
    ...
    public static Map<String, String> filterParams(Map<String, String[]> params, String prefix, String separator) {
        ...
        final String sprefix = prefix + ".";
        params.forEach((k, v) -> {
            if (k.startsWith(sprefix)) {
                filteredMap.put(k.substring(sprefix.length()), Utils.join(v, separator));
            }
        });
        return filteredMap;
    }
    ...
    public static String urlDecodePath(String enc) {
        try {
            return URLDecoder.decode(pattern.matcher(enc).replaceAll("%2B"), Play.defaultWebEncoding);
        } catch (Exception e) {
            return enc;
        }
    }
