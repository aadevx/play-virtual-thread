...
public class LegacyMockMailSystem implements MailSystem {
    ...
    private static String getContent(Part message) throws MessagingException,
        ...
        if (message.getContent() instanceof String string) {
            return message.getContentType() + ": " + string + " \n\t";
            ...
            for (int i = 0; i < part.getCount(); i++) {
                ...
                if (!Message.ATTACHMENT.equals(bodyPart.getDisposition())) {
                    ...
                    text.append("attachment: \n" + "\t\t name: ").append(StringUtils.isEmpty(bodyPart.getFileName()) ? "none" : bodyPart.getFileName()).append("\n").append("\t\t disposition: ").append(bodyPart.getDisposition()).append("\n").append("\t\t description: ").append(StringUtils.isEmpty(bodyPart.getDescription()) ? "none" : bodyPart.getDescription()).append("\n\t");
                }
            }
            return text.toString();
        }
        if (message.getContent() != null && message.getContent() instanceof Part part) {
