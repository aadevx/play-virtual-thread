...
public class DependenciesManager {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // Paths
        final File application = new File(applicationPath);
        final File framework = new File(frameworkPath);
        final File userHome = new File(userHomePath);
    ...
    final DirectoryStream.Filter<Path> dirsToTrim = new DirectoryStream.Filter<Path>() {
    ...
    public void sync(List<File> installed) {
        ...
        for (File path : paths) {
            if (path.exists()) {
                try (DirectoryStream<Path> stream = java.nio.file.Files.newDirectoryStream(path.toPath())){
    ...
    public File install(ArtifactDownloadReport artifact) throws Exception {
        ...
        try {
            ...
            if (!isPlayModule(artifact)) {
                ...
                if (trim) {
                    DirectoryStream<Path> stream = java.nio.file.Files.newDirectoryStream(to.toPath(), dirsToTrim);
                    for (Path entry: stream) {
                        Files.delete(entry.toFile());
                    }
                }
    ...
    public ResolveReport resolve() throws Exception {
        ...
        if (clearcache) {
            ...
            System.out.println("~");
            Files.delete(ivy.getResolutionCacheManager().getResolutionCacheRoot());
