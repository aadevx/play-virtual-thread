...
public class Logger {
     ...
     */
    public static final boolean forceJuli = false;
    /**
     * Will redirect all log from java.util.logging to log4j.
     */
    public static final boolean redirectJuli = false;
    /**
    ...
    static void init(LoggerInit init) {
        ...
        Logger.warn("config log using %s", log4jConf);
        if (log4jConf == null) {
            ...
            context.reconfigure();
            Logger.warn("config log using rootlogger");
   ...
   public static boolean isEnabledFor(org.apache.logging.log4j.Level log4jLevel) {
    ...
    static class LoggerInit{
        ...
        LoggerInit() {
            String log4jPath = Play.configuration.getProperty("application.log.path", "/log4j2.xml");
            log4jConf = Logger.class.getResource(log4jPath);
            if (log4jConf == null) { // try again with the .properties
                log4jPath = Play.configuration.getProperty("application.log.path", "/log4j2.properties");
                log4jConf = Logger.class.getResource(log4jPath);
            }
        }
