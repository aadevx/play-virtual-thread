...
public class GroovyTemplate extends BaseTemplate {
    ...
    public void compile() {
        if (compiledTemplate == null) {
            try {
                ...
                output.add(gclass -> groovyClassesForThisTemplate.add(gclass));
                ...
                try {
                    ...
                    File f = Play.getCompiledTemplate(name);
                    f.getParentFile().mkdirs();
                    FileUtils.write(f, sb.toString(), UTF_8);
                } catch (Exception e) {
    ...
    protected String internalRender(Map<String, Object> args) {
        ...
        try {
            ...
            if (Play.mode == Mode.DEV) {
                ...
                File file = Play.getCompiledTemplate(name);
                if(file.exists())
                    file.delete();
            }
    ...
    protected Throwable cleanStackTrace(Throwable e) {
        ...
        e.setStackTrace(cleanTrace.toArray(new StackTraceElement[0]));
        return e;
    }
    ...
    public abstract static class ExecutableTemplate extends Script {
        ...
        public void invokeTag(Integer fromLine, String tag, Map<String, Object> attrs, Closure body) {
            ...
            if (attrs != null) {
                attrs.forEach((k, v) -> args.put("_" + k, v));
            }
            args.put("_body", body);
            try {
        ...
        public Class __loadClass(String className) throws Exception {
            return Play.classes.loadClass(className);
        }
        ...
        public String __getMessage(Object[] val) {
            ...
            if (val.length == 1) {
                ...
                System.arraycopy(val, 1, args, 0, val.length - 1);
            ...
            public Object invokeMethod(String name, Object param) {
                try {
                    ...
                    try {
                        ...
                        if (param instanceof Object[] obj) {
                            if (obj.length == 1 && obj[0] instanceof Map map) {
                                r = map;
                            } else {
                                ...
                                for (int i = 0; i < obj.length; i++) {
                                    if (obj[i] instanceof Router.ActionDefinition && obj[i] != null) {
                                        Unbinder.unBind(r, obj[i].toString(), i < names.length ? names[i] : "", actionMethod.getAnnotations());
                                    } else if (isSimpleParam(actionMethod.getParameterTypes()[i])) {
                                        if (obj[i] != null) {
                                            Unbinder.unBind(r, obj[i].toString(), i < names.length ? names[i] : "", actionMethod.getAnnotations());
                                        }
                                    } else {
                                        Unbinder.unBind(r, obj[i], i < names.length ? names[i] : "", actionMethod.getAnnotations());
                                    }
                                }
                            }
                        }
