...
public class FastTags {
    ...
    static final Pattern pattern = Pattern.compile("^/lib/[^/]+/app/views/.*");
    ...
    public static void _jsAction(Map<?, ?> args, Closure body, PrintWriter out, ExecutableTemplate template, int fromLine) {
        ...
        html.append("var pattern = '").append(args.get("arg").toString().replace("&amp;", "&")).append("';").append(minimize);
        ...
        if (args.containsKey("customScript")) {
            html.append("val = ").append(args.get("customScript")).append(minimize);
        }
    ...
    public static void _field(Map<?, ?> args, Closure body, PrintWriter out, ExecutableTemplate template, int fromLine) {
        ...
        field.put("errorClass", field.get("error") != null ? "has-error" : "");
    ...
    static boolean _evaluateCondition(Object test) {
        if (test != null) {
            if (test instanceof Boolean bool) {
                return bool.booleanValue();
            } else if (test instanceof String string) {
                return !string.isEmpty();
            } else if (test instanceof Number number) {
                return number.intValue() != 0;
            } else if (test instanceof Collection list) {
                return !list.isEmpty();
    ...
    static String __safe(Template template, Object val) {
        if (val instanceof RawData rawData) {
            return rawData.data;
        }
    ...
    public static void _set(Map<?, ?> args, Closure body, PrintWriter out, ExecutableTemplate template, int fromLine) {
        ...
        for (Map.Entry<?, ?> entry : args.entrySet()) {
            ...
            if (!key.toString().equals("arg")) {
                BaseTemplate.layoutData.get().put(key, (entry.getValue() != null && entry.getValue() instanceof String string) ? __safe(template.template, string) : entry.getValue());
                return;
            }
        }
        // Body case
    ...
    public static void _render(Map<?, ?> args, Closure body, PrintWriter out, ExecutableTemplate template, int fromLine) {
        try {
            ...
            args.remove("arg");
            BaseTemplate t = (BaseTemplate) TemplateLoader.load(name);
            Map<String, Object> newArgs = new HashMap<>((Map<? extends String, ?>) args);
            newArgs.put("_isInclude", true);
            newArgs.put("out", out);
            t.internalRender(newArgs);
        } catch (TemplateNotFoundException e) {
    ...
    public static void _static(Map<?, ?> args, Closure body, PrintWriter out, ExecutableTemplate template, int fromLine) {
        out.println(Play.ctxPath+args.get("arg"));
    }
}
