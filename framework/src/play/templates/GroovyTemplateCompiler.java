...
public class GroovyTemplateCompiler extends TemplateCompiler {
    ...
    public BaseTemplate compile(BaseTemplate template) {
        try {
            ...
            List<Class> extensionsClasses = Play.classes.getAssignableClasses(JavaExtensions.class);
    ...
    static final Pattern patternPlain = Pattern.compile("\"");
    static final Pattern patternPlain2 = Pattern.compile("\r\n");
    static final Pattern patternPlain3 = Pattern.compile("\n");
    @Override
    protected void plain() {
        ...
        skipLineBreak = false;
        text = patternPlain2.matcher(text).replaceAll("\n");
        text = patternPlain3.matcher(text).replaceAll("\\\\n");
    ...
    static final Pattern patternAction = Pattern.compile("^'.*'$");

    @Override
    protected void action(boolean absolute) {
        String action = parser.getToken().trim();
        if (patternAction.matcher(action.trim()).matches()) {
            if (absolute) {
    ...
    static final Pattern patternTag = Pattern.compile("^[_a-zA-Z0-9]+\\s*:.*$");
    static final Pattern patternTag2 = Pattern.compile("[:]\\s*[@]{2}");
    static final Pattern patternTag3 = Pattern.compile("(\\s)[@]{2}");
    static final Pattern patternTag4 = Pattern.compile("[:]\\s*[@]{1}");
    static final Pattern patternTag5 = Pattern.compile("(\\s)[@]{1}");
    static final Pattern patternToken = Pattern.compile("\r");
    static final Pattern patternToken2 = Pattern.compile("\n");

    @Override
    protected void startTag() {
        tagIndex++;
        String tagText = patternToken.matcher(parser.getToken().trim()).replaceAll("");
        tagText = patternToken2.matcher(tagText).replaceAll(" ");
        String tagName;
        String tagArgs;
        ...
        if (tagText.indexOf(" ") > 0) {
            ...
            if (!patternTag.matcher(tagArgs).matches()) {
                tagArgs = "arg:" + tagArgs;
            }
            // We only have to try to replace the following if we find at least one
            // @ in tagArgs..
            if (tagArgs.indexOf('@') >= 0) {
                tagArgs = patternTag2.matcher(tagArgs).replaceAll(":actionBridge._abs().");
                tagArgs = patternTag3.matcher(tagArgs).replaceAll( "$1actionBridge._abs().");
                tagArgs = patternTag4.matcher(tagArgs).replaceAll( ":actionBridge.");
                tagArgs = patternTag5.matcher(tagArgs).replaceAll( "$1actionBridge.");
            }
        } else {
            tagName = tagText;
            tagArgs = ":";
    ...
    protected void endTag() {
        ...
        if (tag.name.equals("doBody")) {
            ...
            try {
                ...
                fastClasses.addAll(Play.classes.getAssignableClasses(FastTags.class));
