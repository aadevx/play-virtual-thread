...
public class UrlEncodedParser extends DataParser {
    ...
    public Map<String, String[]> parse(InputStream is) {
        ...
        try {
            ...
            final String fixcharset = charset;
            params.forEach((k, v) -> {
                try {
                    k = URLDecoder.decode(k, fixcharset);
                } catch (Throwable z) {
                    // Nothing we can do about, ignore
                }
                for (String value : v) {
                    try {
                        Utils.Maps.mergeValueInMap(decodedParams, k, (value == null ? null : URLDecoder.decode(value, fixcharset)));
                    } catch (Throwable z) {
                        // Nothing we can do about, lets fill in with the non decoded value
                        Utils.Maps.mergeValueInMap(decodedParams, k, value);
                    }
                }
            });
