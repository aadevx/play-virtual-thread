...
public class Unbinder {
    ...
    private static void unbindDate(Map<String, Object> result, Object src, Class<?> srcClazz, String name, Annotation[] annotations) {
                ...
                for (Annotation annotation : annotations) {
                    if (annotation.annotationType().equals(As.class)) {
                        if (Calendar.class.isAssignableFrom(src.getClass())) {
                              result.put(name, DateTimeFormatter.ofPattern(((As) annotation).value()[0]).format(Time.toLocalDateTime((Calendar) src)));
                        } else {
                            result.put(name, DateTimeFormatter.ofPattern(((As) annotation).value()[0]).format(Time.toLocalDateTime((Date) src)));
                        }
                        isAsAnnotation = true;
                    }
                }
        ...
        if (!isAsAnnotation) {
            ...
            if (Calendar.class.isAssignableFrom(src.getClass())) {
                result.put(name, DateTimeFormatter.ofPattern(I18N.getDateFormat()).format(Time.toLocalDateTime((Calendar) src)));
            } else {
                result.put(name, DateTimeFormatter.ofPattern(I18N.getDateFormat()).format(Time.toLocalDateTime((Date) src)));
            }
        }
    }
