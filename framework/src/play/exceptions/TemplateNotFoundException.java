...
public class TemplateNotFoundException extends PlayException implements SourceAttachment {
    ...
    public TemplateNotFoundException(String path, ApplicationClass applicationClass, Integer line) {
        ...
        if (applicationClass != null && applicationClass.javaFile != null && applicationClass.source() != null) {
            this.sourceFile = FileUtils.relativePath(applicationClass.javaFile);
            this.source = Arrays.asList(applicationClass.source().split("\n"));
        }
        else {
