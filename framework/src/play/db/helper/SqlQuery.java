...
public abstract class SqlQuery {
    ...
    public static String inlineParam(Object param) {
        ...
        String str;
        if (param instanceof String string) str = quote(string);
        else if (param instanceof Iterable<?> iterable) {
            Concat list = new Concat("(", ", ", ")");
            for (Object p : iterable) list.append(inlineParam(p));
            str = list.toString();
        } else if (param instanceof Object[] obj) {
            Concat list = new Concat("(", ", ", ")");
            for (Object p : obj) list.append(inlineParam(p));
            str = list.toString();
        } else if (param instanceof Enum<?> enumParam) {
            str = quote(enumParam.toString());
