...
/*public static Response POST(Request request, Object url, Map<String, String> parameters, Map<String, File> files) {
        ...
        for (Map.Entry<String, String> entry : parameters.entrySet()) {
            StringPart stringPart = new StringPart(entry.getKey(), entry.getValue(), request.contentType, Charset.forName(request.encoding));
        ...
        *//*
        ...
        ByteBuf baos = null;
        try {
            requestEntity = MultipartUtils.newMultipartBody(parts, null);
            Map<String, Http.Header> contentTypeHeader = new HashMap<String, Http.Header>();
            contentTypeHeader.put("content-type", new Http.Header("content-type", requestEntity.getContentType()));
            request.headers.putAll(contentTypeHeader);
            ...
            baos = Unpooled.buffer(0, (int)contentLength);
            requestEntity.transferTo(baos);
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
        // InputStream body = new ByteArrayInputStream(array != null ? array :
        // new byte[0]); // ^1
        InputStream body = new ByteArrayInputStream(baos != null ? baos.array() : new byte[0]);
    ...
    public static void makeRequest(final Request request, final Response response) {
        ...
        TestEngine.functionalTestsExecutor.submit(new Invocation() {
        ...
        try {
            ...
            response.cookies.forEach((k, v) -> {
                // If Max-Age is unset, browsers discard on exit; if 0, they discard immediately.
                if (v.maxAge == null || v.maxAge > 0) {
                    savedCookies.put(k, v);
                } else {
                    // cookies with maxAge zero still remove a previously existing cookie, like PLAY_FLASH.
                    savedCookies.remove(k);
                }
            });
            response.out.flush();
        } catch (Exception ex) {
