...
The CRUD (Create, Read, Update, Delete) module generates a fully usable web interface for your Model objects.
...
The CRUD module provides a user-interface for your application’s model classes. We will start by creating a User model class, which is a entity:
...
bc. protected static ObjectType createObjectType(Class<? extends BaseTable> type) {
...
bc.   public class CustomAdminCompany extends CRUD {
   protected static ObjectType createObjectType(Class<? extends BaseTable> type) {
      return new VersionObjectType(type);
   }
   ...
   public static class VersionObjectType extends ObjectType {
      ...
      public VersionObjectType(Class<? extends BaseTable> modelClass) {
         super(modelClass);
