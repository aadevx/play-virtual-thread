...
h2. <a name="objectbinding"> object binding</a>

You can automatically bind a object using the HTTP to Java binding.
...
You can use binding to modify complete object graphs in the same way as POJO mapping works, but you have to supply the ID for each sub object you intend to modify:
...
bc. public class User extends BaseTable {
...
To serve binary data use the @renderBinary@ method. For example, if you have a @User@ model with a @play.db.jpa.Blob photo@ property, add a controller method to load the model object and render the image with the stored MIME type:
